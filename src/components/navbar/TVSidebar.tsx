...
export function TVSidebar() {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 bottom-0 h-screen z-50 transition-all mr-2 duration-300 ease-in-out flex flex-col items-start pt-8 pb-8 border-r-0 overflow-hidden pointer-events-none ${isExpanded
        ? "w-[280px] mr-3 bg-gradient-to-r from-black/80 via-black/88 via-[60%] to-transparent"
        : "w-[80px] bg-gradient-to-r from-black via-black/90 to-black/20"
        }`}
    >
      <div
      ...
      >
          ...
          {tvNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={item.path}
                className={`focusable flex items-center gap-5 py-3 w-full outline-none transition-colors duration-300 group pl-4 ${isActive
        ...
        <div className="w-full border-t border-zinc-800/40 pt-6 flex flex-col items-center ml-5">
          {user?.membershipStatus === "active" ? (
            ...
            <div
              className={`text-[10px] text-zinc-500 font-bold uppercase tracking-wider text-left w-full transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${isExpanded ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-3 pointer-events-none"
                }`}
            >
              {currentPlanName}
            </div>
          ...
          >
            ...
            <div
              className={`flex flex-col text-left transition-all duration-300 ease-in-out ${isExpanded ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-1 pointer-events-none"
                }`}
            >
